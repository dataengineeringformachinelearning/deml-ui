...
3. **Geist only** — `--font-display`, `--font-secondary`, and body all resolve to Geist. Do not ship Syne/Fraunces mixes.
4. **8px grid** — spacing and tile rhythm via `--grid`, `--tile-gap`, `--tile-row-unit`.
5. **Fluid equal cells** — `grid-auto-rows: minmax(var(--tile-row-unit), auto)` so charts grow; never squash with fixed-only rows.
6. **Charts** — keep `--chart-aspect: 2.4`; never squash or stretch plots.
